...
import { useAccessDayForm } from './hooks/useAccessDayForm';
...
export default function AccessDayForm({onOpenChange} :Props) {
  const { register, onSubmit, errors, control, isSubmitting, handleSubmit, trigger, watch, setValue } = useAccessDayForm();
  ...
  return (
      ...
      <div className="space-y-1 sm:space-y-2">
        <Label htmlFor="areaOfInterest" className="text-xs sm:text-sm font-medium font-inter">
        ...
        />
        {errors.areaOfInterest && <p className="text-xs text-red-500">{errors.areaOfInterest.message}</p>}
      </div>
