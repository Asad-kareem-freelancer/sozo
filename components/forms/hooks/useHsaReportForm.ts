...
export type HsaReportFormData = z.infer<typeof hsaReportSchema>;

export function useHsaReportForm() {
  ...
  const {
    ...
    setValue,
    trigger,
  } = useForm<HsaReportFormData>({
    resolver: zodResolver(hsaReportSchema),
    defaultValues: {
      firstName: '',
      lastName: '',
      email: '',
  ...
  const onSubmit = async (data: HsaReportFormData, callback?: (v:boolean) => void) => {
    setIsSubmitting(true);

    try {
      const result = await submitHsaReportForm(data);

      if (result.success) {
        reset();
        callback?.(false);

        // Show success alert with download link - URL to be updated for January 2026 launch
        const pdfUrl = 'https://d9gpta2bry95a.cloudfront.net/publications/HSA_v1.pdf';
        const reportTitle = 'Health Systems Assurance (HSA) — Volume 1 (2026)';
