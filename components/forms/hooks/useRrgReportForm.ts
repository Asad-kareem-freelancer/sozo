...
export type RrgReportFormData = z.infer<typeof rrgReportSchema>;

export function useRrgReportForm() {
  ...
  const {
    ...
    setValue,
    trigger,
  } = useForm<RrgReportFormData>({
    resolver: zodResolver(rrgReportSchema),
    defaultValues: {
      firstName: '',
      lastName: '',
      email: '',
  ...
  const onSubmit = async (data: RrgReportFormData, callback?: (v:boolean) => void) => {
